...
public class MethylCall {
    ...
    static int modbaseprobs_threshold = 128;
        ...
        try {
            for(int i = 0; i<argv.length-1; i++){
                if(argv[i].equals("--rate")){
                    ...
                    methylkit = true;
                }else if(argv[i].equals("--mod-threshold")){
                    modbaseprobs_threshold = Integer.parseInt(argv[i+1]);
                }
            }
            ...
            for(File file: files){
                pool.submit(new Runnable(){
                    public void run(){
                        new Histogram().run(file.getPath(), rate, rate_threshold, methylkit, modbaseprobs_threshold);
                        synchronized(latch){
                            latch.countDown();
                        }
